...
#           e += numpy.array([eyesX1, eyesY1, eyesX1, eyesY1],dtype=numpy.int32);
        ...
        print( 'final rects={}'.format(rects))
...
class Util:
    ...
    def threshold(img, thresh):
        ...
        r, t = cv2.threshold(inverted, thresh, 0, type=cv2.CV_THRESH_TOZERO);
    ...
    def equalizeHSV(img, equalizeH=False, equalizeS=False, equalizeV=True):
        ...
        imgHSV = cv2.cvtColor(img,cv2.CV_BGR2HSV);
        ...
        hsv = cv2.merge([h,s,v]);
        bgr = cv2.cvtColor(hsv,cv2.CV_HSV2BGR);
        return bgr;
...
class Display:
    ...
    def renderEyes(self, frame, model):
        ...
        if len(eyeRects[0]) is 4:
            ...
            eyeLeftExpanded = cv2.cvtColor(eyeLeftExpanded,cv2.CV_BGR2GRAY);
            ...
            # Grayscale Eye
            eyeLeftBW = cv2.cvtColor(eyeLeftIMG,cv2.CV_BGR2GRAY);
...
#           thresholded = Util.threshold(B,200);
            ...
            eyeLeftFeatureMap = cv2.cvtColor(eyeLeftExpanded,cv2.CV_GRAY2BGR);
...
#           eyeLeftCorners = cv2.cvtColor(eyeLeftEqualized,cv2.CV_GRAY2BGR);
...
#           cv2.moveWindow("eyeLeftCorners",0,750);
            ...
            circles = cv2.HoughCircles(eyeLeftBlurredBW, cv2.CV_HOUGH_GRADIENT, 2.5, minDistance, param1=30, param2=30,minRadius=irisMinRadius,maxRadius=irisMaxRadius);
            
            eyeLeftBW_C = cv2.cvtColor(B,cv2.CV_GRAY2BGR);
...
class Capture:
    ...
    def __init__(self, scaleFactor=1):
        ...
        if scaleFactor is not 1:
            ...
            self.camera.set(cv2.CV_CAP_PROP_FRAME_HEIGHT,scaledHeight);
            self.camera.set(cv2.CV_CAP_PROP_FRAME_WIDTH,scaledWidth);
